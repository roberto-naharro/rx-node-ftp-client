...
var Client = /** @class */ (function () {
    ...
    Client.prototype.checkTimezone = function () {
        ...
        var manageError = function (err) { return _this.testEmitError(err, 'Error - ' +
            JSON.stringify(err), 'debug', function () { return rxjs_1.of(null); }); };
        return rxjs_1.race(this.errorObservable, rxjs_1.of(null).pipe(operators_1.switchMapTo(rxjs_1.bindNodeCallback(this.ftp.put)
            .call(this.ftp, new Buffer('blank'), timestampPath).pipe(operators_1.catchError(function (err) {
            return manageError(err);
        }))), operators_1.switchMapTo(rxjs_1.bindNodeCallback(this.ftp.list)
            .call(this.ftp, timestampPath).pipe(operators_1.catchError(function (err) {
            return manageError(err);
    ...
    Client.prototype.deleteFiles = function (filesToDelete, baseDir) {
        ...
        return rxjs_1.iif(function () { return _this.status === 'disconnected'; }, rxjs_1.throwError(new Error('Not connected')), this.forkJoinLimit(this.MAX_CONNECTIONS, _.map(filesToDelete, function (file) {
            return _this.deleteRemoteFile(file, baseDir);
    ...
    Client.prototype.deleteLocalFiles = function (filesToDelete) {
        ...
        return this.forkJoinLimit(this.MAX_CONNECTIONS, _.map(filesToDelete, function (file) {
            return rxjs_1.bindNodeCallback(fs.unlink).call(_this, file);
        })).pipe(operators_1.mapTo(null));
    };
    ...
    Client.prototype.createLocalDirectories = function (dirs, baseDir, dest) {
        ...
        dirs.forEach(function (dir) {
            ...
            out.push(_this.fileExist(dirName).pipe(operators_1.switchMap(function (exist) {
                if (!exist) {
                    return rxjs_1.bindNodeCallback(fs.mkdir).call(_this, dirName).pipe(operators_1.tap(function () { return _this.log('Created directory ' + dirName, 'debug'); }));
                }
                return rxjs_1.of(null);
            })));
        });
            ...
            return _this.fileExist(fileName).pipe(operators_1.switchMap(function (exist) {
                if (exist) {
                    if (options.overwrite === 'older') {
                        return rxjs_1.bindNodeCallback(fs.stat).call(fs, fileName).pipe(operators_1.switchMap(function (stat) {
    ...
    Client.prototype.lookForRemoteContent = function (dir) {
        ...
        var dirs = [];
        var files = {};
        return rxjs_1.bindNodeCallback(this.ftp.list).call(this.ftp, dir).pipe(operators_1.tap(function () {
        ...
        return new rxjs_1.Observable(function (observer) {
            ...
            _this.ftp.get.call(_this.ftp, file, function (err, stream) {
                if (err && err.message !== 'Unable to make data connection') {
                    _this.testEmitError(new Error('Error downloading file ' + file + ' - '
    ...
    Client.prototype.upload = function (patterns, dest, options) {
        ...
        return this.cwd(dest).pipe(operators_1.tap(function () { return _this.log('Moved to directory ' + dest, 'debug'); }), operators_1.switchMap(function () {
            ...
            sources(toDelete);
            _this.log('Found ' + files.length + ' files and ' + dirs.length +
            ...
            _this.log('Finished uploading ' + result.uploadedFiles.length + ' of ' +
    ...
    Client.prototype.download = function (source, dest, options) {
        ...
        return this.fileExist(dest).pipe(operators_1.switchMap(function (exist) {
            ...
            return _this.forkJoinLimit(_this.MAX_CONNECTIONS, _.map(_.keys(files), function (file) { return _this.dowloadSingleFile(file, source, dest).pipe(operators_1.tap(function (fileDownloaded) {
                if (fileDownloaded) {
            ...
            _this.log('Finished downloading ' + result.downloadedFiles.length +
