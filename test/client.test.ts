...
describe('Test upload', function () {
  const testFiles = ['./test.txt', './dir', './dir/*'];
  ...
  afterEach(function (done) {
    ...
    ftpClient
      ...
      .pipe(
        ...
        switchMapTo(ftpClient.disconnect()),
      )
      .subscribe({
        error: (err) => done(err),
        complete: () => done(),
  ...
  it(`upload one file to the test server`, function (done) {
    ftpClient
      .upload(path.join(__dirname, testFiles[0]), remotePath, {
        baseDir: __dirname,
      })
      .subscribe(
        (results) => {
  ...
  it(`upload multiple files to the test server`, function (done) {
    ftpClient
      .upload(
        _.map(testFiles, (t) => path.join(__dirname, t)),
        remotePath,
        { baseDir: __dirname },
      )
      .subscribe(
...
describe('Test download', function () {
  ...
  beforeEach(function (done) {
    ...
    ftpClient
      ...
      .pipe(
        ...
        tap(() =>
          console.info(
            'Before - Files uploaded, creating folder ' + downloadPath,
          ),
        ),
        switchMapTo(
          bindNodeCallback(fs.mkdir)
            .call(this, path.join(__dirname, downloadPath))
            .pipe(
              catchError((err) => {
                if (err.code === 'EEXIST') {
                  return of(null);
  ...
  afterEach(function (done) {
    ...
    ftpClient
      ...
      .pipe(
        ...
        catchError((err) => {
          return endObservable.pipe(mapTo(throwError(err)));
        }),
      )
      .subscribe({
        error: (err) => done(err),
  ...
  it(`Download a folder from the test server`, function (done) {
    ...
    ftpClient
      .download(remotePath, path.join(__dirname, downloadPath))
      .pipe(
        catchError((err) => {
          return ftpClient.disconnect().pipe(switchMapTo(throwError(err)));
        }),
      )
      .subscribe(
        (results) => {
